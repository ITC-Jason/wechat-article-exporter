...
// 创建自定义本地化，中文使用 AG Grid 内置中文包，英文回落到 AG Grid 默认英文。
function getLocaleText(t: Translate, locale: string) {
  if (locale === 'zh-CN') {
    return {
      ...AG_GRID_LOCALE_CN,
      columns: t('grid.columnsPanel'),
    };
  }
 ...
 * Grid表格公共配置
 */
export function getSharedGridOptions(t: Translate, locale: string): GridOptions {
  return {
    localeText: getLocaleText(t, locale),
    rowNumbers: {
      resizable: true,
      minWidth: 80,
      maxWidth: 120,
...
export const sharedGridOptions: GridOptions = getSharedGridOptions(key => key, 'zh-CN');
